...
public class MessagesRepositoryJdbcImpl implements MessagesRepository {
    ...
    public void update(Message message) {
        if(message != null){
            Long author_id = message.getAuthor() == null ? null : message.getAuthor().getId();
            Long room_id = message.getRoom() == null ? null : message.getRoom().getId();

            String UPDATE_QUERY = "UPDATE chat.messages set " +
                    String.format("author = %s, room = %s, text = '%s', dateTime = %s where id = %s;",
                            author_id, room_id, message.getText(), message.getDateTime(), message.getId());
            try (Connection con = dataSource.getConnection()) {
                try(PreparedStatement statement = con.prepareStatement(UPDATE_QUERY,
                        Statement.RETURN_GENERATED_KEYS)){
                    if (statement.executeUpdate() == 0) {
                        throw new SQLException("Updated fail.");
                    }
